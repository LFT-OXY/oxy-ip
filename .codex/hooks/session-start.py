...
# -*- coding: utf-8 -*-
"""
Codex Session Start Hook - Inject ATW context into Codex sessions.
...
def configure_project_encoding(project_dir: Path) -> None:
    """Reuse ATW' shared Windows stdio encoding helper before JSON output."""
...
def _resolve_active_task(atw_dir: Path, hook_input: dict):
    scripts_dir = atw_dir / "scripts"
    if str(scripts_dir) not in sys.path:
        sys.path.insert(0, str(scripts_dir))
    from common.active_task import resolve_active_task  # type: ignore[import-not-found]

    return resolve_active_task(atw_dir.parent, hook_input, platform="codex")
...
def _get_task_status(atw_dir: Path, hook_input: dict) -> str:
    active = _resolve_active_task(atw_dir, hook_input)
    if not active.task_path:
        return (
            "Status: NO ACTIVE TASK\n"
            "Next: Classify the current turn and ask for task-creation consent "
            "before creating any ATW task."
        )
    ...
    if active.stale or not task_dir.is_dir():
        return (
            ...
            "Next: Task directory not found. Run: python3 ./.atw/scripts/task.py finish"
        )
    ...
    if task_status == "completed":
        return (
            f"Status: COMPLETED\nTask: {task_title}\n"
            f"Next: Archive with `python3 ./.atw/scripts/task.py archive {task_dir.name}` "
            "or start a new task."
        )
    ...
    present = [
        name
        for name in ("prd.md", "implement.jsonl", "check.jsonl")
        if (task_dir / name).is_file()
    ]
    present_line = ", ".join(present) if present else "none"

    if not has_prd:
        # 这条不以任务状态为门，任何阶段缺 prd.md 都会进——所以状态名要照实报，
        # 不能像以前那样硬编码成一个词表里已经没有的阶段。
        return (
            f"Status: {task_status.upper()}\nTask: {task_title}\nPresent: {present_line}\n"
            "Next: Load atw-brainstorm and write prd.md before task.py start."
        )

    # 上面那条 `not has_prd` 不以任务状态为门，任何阶段缺 prd.md 都会进，是活分支。
    return (
        f"Status: {task_status.upper()}\nTask: {task_title}\nPresent: {present_line}\n"
...
def _collect_spec_index_paths(atw_dir: Path) -> list[str]:
    paths: list[str] = []
...
def main() -> None:
    ...
    spec_index_paths = _collect_spec_index_paths(atw_dir)
...
""")
    ...
    output.write(_build_workflow_toc(atw_dir / "workflow.md"))
    output.write("\n</atw-workflow>\n\n")
...
Context loaded. Follow <task-status>. Load workflow/spec/task details only when needed.
    ...
    result = {
        "suppressOutput": True,
        "systemMessage": f"ATW context injected ({len(context)} chars)",
        "hookSpecificOutput": {
