...
These product-neutral examples compare a raw agent response with Noob Mode. They are written in English for compactness; preserve the meaning and structure, then localize every explanation and heading to the user’s active language.
...
⚠️ RISK: 🟢 Low
This command only searches local files — it doesn't change, delete, or upload anything.
...
⚠️ RISK: 🟢 Low
This reads already-public information from GitHub. It doesn't post, change, or submit anything.
...
If your agent asks for approval at any step, I'll explain that approval before you decide.
...
```
Error: EACCES: permission denied, open '/usr/local/lib/node_modules/example-agent/package.json'
...
Each item includes a plain-language explanation and recommended next steps.
