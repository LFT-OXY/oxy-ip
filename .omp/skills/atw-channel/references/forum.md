...
```bash
atw channel create design-feedback \
  --type forum \
  --scope global \
...
```bash
atw channel post design-feedback opened \
  --scope global \
  --as main \
...
atw channel post design-feedback comment \
  --scope global \
  --as reviewer \
  --thread login-empty-state \
  --text-file /tmp/review.md

atw channel post design-feedback status \
  --scope global \
  --as main \
  --thread login-empty-state \
  --status closed

atw channel post design-feedback summary \
  --scope global \
  --as main \
  --thread login-empty-state \
  --summary "Adopted the option-B layout; ticket ATW-123 owns the fix."
```
...
```bash
atw channel messages design-feedback --scope global
atw channel forum design-feedback --scope global --status open
atw channel thread design-feedback login-empty-state --scope global
atw channel messages design-feedback --scope global --raw --thread login-empty-state
```
...
atw channel context add design-feedback \
  --scope global \
...
atw channel context add design-feedback \
  --scope global \
  --thread login-empty-state \
  --file "$PWD/.atw/tasks/05-13-login-redesign/prd.md"
```
...
```bash
atw channel context list design-feedback --scope global
atw channel context list design-feedback --scope global --thread login-empty-state --raw
```
...
```bash
atw channel context delete design-feedback \
  --scope global \
  --thread login-empty-state \
  --raw "stale note"
```
...
```bash
atw channel title set design-feedback \
  --scope global \
  --title "Design feedback board"

atw channel title clear design-feedback --scope global
```
...
```bash
atw channel thread rename design-feedback old-key new-key \
  --scope global \
  --as main
```
...
```bash
atw channel create release-notes \
  --type forum \
  --scope global \
...
atw channel post release-notes opened \
  --scope global \
  --as main \
