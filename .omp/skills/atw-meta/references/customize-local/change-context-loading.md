...
1. `.atw/workflow.md`
2. `.atw/scripts/get_context.py`
3. `.atw/scripts/common/session_context.py`
4. `.atw/scripts/common/task_context.py`
5. `.atw/scripts/common/active_task.py`
...
| Source                              | Purpose                                      |
| ----------------------------------- | -------------------------------------------- |
| `.atw/workflow.md`                  | Workflow and next-action hints.              |
| `.atw/tasks/<task>/prd.md`          | Current task requirements.                   |
| `.atw/tasks/<task>/implement.jsonl` | Spec/research to read before implementation. |
| `.atw/tasks/<task>/check.jsonl`     | Spec/research to read during review.         |
| `.atw/spec/`                        | Project specs.                               |
| `.atw/workspace/`                   | Session records.                             |
...
| Change hints on each user input              | `[workflow-state:STATUS]` block in `.atw/workflow.md`. The `inject-workflow-state` hook is parser-only and reads the block verbatim. |
...
```jsonl
{"file": ".atw/spec/backend/index.md", "reason": "Backend conventions"}
{"file": ".atw/tasks/04-28-x/research/api.md", "reason": "API research"}
```
...
- `.atw/scripts/common/session_context.py`
...
- agent pull: edit the read steps in the corresponding `atw-implement-agent` / `atw-review` agent file.
...
python3 ./.atw/scripts/task.py list-context <task>
