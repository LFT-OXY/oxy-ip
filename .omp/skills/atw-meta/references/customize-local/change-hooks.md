...
Hooks are the automation layer that connects a platform to ATW. When the user wants to change "when context is injected," "how shell commands inherit a session," or "which files are read before an agent starts," hooks are usually the edit point.
...
3. `.atw/scripts/common/active_task.py`
4. `.atw/scripts/common/session_context.py`
5. `.atw/workflow.md`
...
| Hook                 | Purpose                                                             |
| -------------------- | ------------------------------------------------------------------- |
| session-start        | Injects an ATW overview when a session starts, clears, or compacts. |
...
3. Read the hook script and identify inputs, outputs, and called `.atw/scripts/`.
4. Modify hook behavior.
5. If the hook depends on workflow content, synchronize `.atw/workflow.md`.
...
If the hook ultimately calls `.atw/scripts/get_context.py` or `session_context.py`, editing the local script is usually more robust than hard-coding content in the hook.
...
- Hooks should read project-local `.atw/`; they should not depend on ATW upstream source paths.
