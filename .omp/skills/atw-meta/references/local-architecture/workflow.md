...
`.atw/workflow.md` is the ATW workflow source of truth inside the user project. An AI does not need ATW source code to understand how the current project should move tasks forward; this file is enough.

## File Responsibilities

`.atw/workflow.md` has three responsibilities:
...
- Platforms with sub-agent support: implementation runs in the main session via `/atw-implement`; that run dispatches `atw-review` for the two review axes. `atw-implement-agent` is a reserved role file and is not dispatched.
- Platforms without sub-agent support: the main session reads skills such as `atw-before-dev`, then executes directly.
...
| `task_error` | The active task record could not be read; repair it before doing anything else.         |
| `discover`   | Open requirements are still being worked through; no spec yet.                          |
| `specify`    | Turning the discover findings into `prd.md`, then curating review context.              |
| `slice`      | Slicing the confirmed `prd.md` into tickets. Optional — single-session work skips it.   |
| `implement`  | Implementation and review, one ticket at a time.                                        |
| `accept`     | The work is done and waiting for the user's acceptance before archive.                  |
| `completed`  | Terminal. Written by `task.py archive`, which also moves the task out of `.atw/tasks/`. |
...
| Change the default implementation/review path | Update Phase 2 and skill routing.                                                                                                                                                                                                                                                                                         |
| Change the wrap-up flow                       | Update Phase 3 and `finish-work` related descriptions. Note the current split: Phase 3 is acceptance (**3.1**, stop ④) then archive (**3.2**) — there is no wrap-up commit step, because the `implement` stage commits inside each ticket's chain. The `finish-work` command refuses to run if the working tree is dirty. |
| Change platform differences                   | Update routing descriptions grouped by platform.                                                                                                                                                                                                                                                                          |

After editing, make the AI reread `.atw/workflow.md`; do not assume the flow from the old conversation is still valid.
...
- skills, such as `atw-brainstorm` and `atw-code-review`.
