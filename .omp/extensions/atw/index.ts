...
function findProjectRoot(startDir: string): string | null {
  ...
  while (true) {
    if (existsSync(join(current, ".atw"))) return current;
...
function deriveContextKey(ctx?: {
  ...
  const override = process.env.ATW_CONTEXT_ID?.trim();
...
function parseChannelTrustSection(content: string): {
  ...
  for (const raw of lines) {
    ...
    const boolMatch = trimmed.match(/^ {2}auto_trust_atw_symlinks:\s*(.+)$/);
    if (boolMatch) {
      ...
      if (val === "false") autoTrustSymlinks = false;
        ...
        process.stderr.write(
          `[channel] channel.auto_trust_atw_symlinks: invalid value '${val}', ignoring\n`,
        );
      continue;
    }
  }
...
function resolveTrustedRoots(projectRoot: string): string[] {
  const configPath = join(projectRoot, ".atw", "config.yaml");
  let config: { trustedDirs: string[]; autoTrustSymlinks?: boolean } = {
    trustedDirs: [],
  };
  ...
  if (config.autoTrustSymlinks !== false) {
    for (const entryName of AUTO_TRUST_ENTRIES) {
      const entryPath = join(projectRoot, ".atw", entryName);
      try {
...
function resolveProjectFile(
  ...
  try {
    ...
    if (trustedRoots.some((root) => isInsideRoot(root, targetReal)))
      return targetReal;
    return null;
  } catch {
    return null;
  ...
  if (taskDir) {
    ...
    const taskLabel =
      ...
      !isAbsolute(taskRelative)
        ? taskRelative
        : `.atw/tasks/${basename(taskDir)}`;
    const withinTask = relative(taskDir, filePath).split("\\").join("/");
    if (
      withinTask &&
      !withinTask.startsWith("../") &&
      !isAbsolute(withinTask)
    ) {
      return `${taskLabel}/${withinTask}`;
    }
  }
  try {
    return relative(realpathSync(projectRoot), realpathSync(filePath))
      .split("\\")
      .join("/");
  } catch {
...
function resolveActiveTaskStatus(
  ...
  const sessionsDir = join(projectRoot, ".atw", ".runtime", "sessions");
  ...
  // written before that fix can still hold one, and `atw update` does not
  // rewrite session files — so a poisoned pointer outlives the upgrade that
  // closed the writer.
  const taskDir = resolveProjectFile(
    projectRoot,
    currentTask,
    resolveTrustedRoots(projectRoot),
  );
  ...
  return {
    // 兜底是 task_error 不是某个合法档位：status 缺失/非字符串意味着记录坏了，
    // 默认成一个档位会把坏记录当成新任务静默正常化，用户永远不知道 task.json 有问题。
    status:
      typeof taskData.status === "string" ? taskData.status : "task_error",
    taskDir,
...
function buildSessionContext(
  ...
  const script = join(projectRoot, ".atw", "scripts", "get_context.py");
  ...
  try {
    const result = spawnSync("python3", [script], {
      ...
      encoding: "utf-8",
      env: contextKey
        ? { ...process.env, ATW_CONTEXT_ID: contextKey }
        : process.env,
...
// Agent names mirror `AGENTS_ALL` in shared-hooks/inject-subagent-context.py —
// that file is the authority. The standalone research sub-agent was retired
// (the skill of the same name stays); jsonl file names are unchanged.
type AgentType = "atw-implement-agent" | "atw-review" | null;

function taskContextJsonlNames(agentType?: AgentType): string[] {
  if (agentType === "atw-implement-agent") return ["implement.jsonl"];
  if (agentType === "atw-review") return ["check.jsonl"];
  return ["implement.jsonl", "check.jsonl"];
}

function taskContextInputPaths(
  projectRoot: string,
  taskDir: string,
  agentType?: AgentType,
): string[] {
  ...
  const paths = new Set<string>([
    join(projectRoot, ".atw", "config.yaml"),
    join(taskDir, "prd.md"),
  ...
  for (const jsonlName of taskContextJsonlNames(agentType)) {
    ...
    for (const line of lines) {
      try {
        ...
        if (candidatePath && isInsideRoot(resolve(projectRoot), candidatePath))
...
function taskContextSignature(
  projectRoot: string,
  taskDir: string,
  agentType?: AgentType,
): string {
  return taskContextInputPaths(projectRoot, taskDir, agentType)
    .map((filePath) => {
      try {
...
function readContextInjectionLimits(
  projectRoot: string,
): ContextInjectionLimits {
  const limits = { ...DEFAULT_CONTEXT_INJECTION_LIMITS };
  let config = "";
  try {
    config = readFileSync(join(projectRoot, ".atw", "config.yaml"), "utf-8");
  } catch {
    return limits;
  }
  ...
  for (const rawLine of config.split(/\r?\n/)) {
    ...
    (limits as unknown as Record<string, number>)[match[1]!] = Number.parseInt(
      rawValue,
      10,
    );
  }
...
class ContextBudget {
  ...
  hasRoom(bytes: number): boolean {
    return (
      ...
      (this.maxTotalBytes <= 0 ||
        this.totalBytesUsed + bytes <= this.maxTotalBytes)
    );
  }
  ...
  private emitTerminalSummary(): string | null {
    ...
    this.stopped = true;
    const summary =
      "[ATW: context limit reached; omitted entries remain authoritative on disk and require_read paths above.]";
...
function utf8Status(data: Buffer): Utf8Status {
  for (let index = 0; index < data.length; index++) {
    ...
    for (
      let continuation = index + 2;
      continuation < index + length;
      continuation++
    ) {
...
function truncateUtf8(data: Buffer, cap: number): Buffer {
  ...
  if (utf8Status(data.subarray(0, cap)) === "valid")
    return data.subarray(0, cap);
...
function omittedNotice(
  ...
  return `### ${file} [omitted]\n\n[ATW: omitted (${reason}) — ${file} (${sizeText}); required_read: ${file}]`;
}
...
function materialize(
  ...
  if (!file) {
    return {
      block: null,
      notice: omittedNotice(
        displayPath,
        null,
        `${kind} is missing or unreadable`,
      ),
    };
  }
  ...
  if (
    ...
    (encodingStatus === "incomplete" && size <= data.length)
  ) {
    return {
      block: null,
      notice: omittedNotice(
        displayPath,
        size,
        `binary or non-UTF-8 ${kind}: ${reason}`,
      ),
    };
  }
  ...
  if (truncated.length < size) {
    status = "truncated";
    content += `\n[ATW: truncated at ${maxBytes} bytes — read ${displayPath} for the full content]`;
  }
  return {
    ...
    notice: omittedNotice(
      displayPath,
      size,
      `total context limit reached: ${reason}`,
    ),
  };
}
...
function readJsonlLines(
  jsonlPath: string,
  displayPath: string,
): { lines: string[]; omitted: string | null } {
  let fd: number | null = null;
  try {
    ...
    if (bytesRead > MAX_JSONL_BYTES || size > MAX_JSONL_BYTES) {
      return {
        lines: [],
        omitted: omittedNotice(
          displayPath,
          size,
          `manifest exceeds ${MAX_JSONL_BYTES} byte parse limit`,
        ),
      };
    }
...
function buildTaskContext(
  ...
  const prefix =
    "<task-context>\nContext is bounded by .atw/config.yaml. Files marked [truncated] or [omitted] remain authoritative on disk; use their required_read path before relying on missing detail.\n\n";
  const suffix = "\n</task-context>";
  ...
  for (const jsonlName of jsonlNames) {
    ...
    const relativeJsonlPath = displayProjectPath(
      projectRoot,
      jsonlPath,
      taskDir,
    );
    ...
    for (const line of manifest.lines) {
      ...
      try {
        ...
        const sectionPrefix = sectionHeaderEmitted
          ...
          : `${parts.length > 0 ? "\n\n" : ""}## ${jsonlName}\n\n`;
        const output = budget.emitCandidate(
          materialized.block === null
            ? null
            : `${sectionPrefix}${materialized.block}`,
  ...
  if (
    ...
    Buffer.byteLength(context, "utf-8") <= limits.max_total_bytes
  )
    return context;
  ...
  const body = truncateUtf8(
    Buffer.from(`${prefix}${parts.join("")}`, "utf-8"),
    bodyLimit,
  ).toString("utf-8");
  return `${body}${suffix}`;
}
...
// the skip keyword defaults to "no-atw"; an explicit "" disables the
// escape hatch entirely.
const DEFAULT_PROMPT_INJECTION_SKIP_KEYWORD = "no-atw";
...
function readPromptInjectionSkipKeyword(projectRoot: string): string {
  ...
  for (const rawLine of config.split(/\r?\n/)) {
    ...
    // 有键无值（`skip_keyword:`）时 parse_simple_yaml 给出的是 {} 而不是字符串，
    // Python 侧的 isinstance(raw, str) 因此回落默认值。这里不回落的话关键词会
    // 变成空串，而空关键词永不匹配——逃生舱静默永久失效，方向和基准正好相反。
    // 显式写的 `""` 是另一回事：那是用户主动关掉逃生舱，要原样保留。
    if (!rawValue) return DEFAULT_PROMPT_INJECTION_SKIP_KEYWORD;
    // 不再对去引号后的值 trim：`"  x  "` 在基准里前后空格是保留的
    // （实测 parse_simple_yaml 给出 '  x  '）。外层空格已在上一行 trim 掉，
    // 那一次对应基准对裸值的处理；再 trim 一次就会剥掉引号内的内容。
...
// as a word char so "no-atwx" / "xno-atw" / "foo-no-atw" don't
...
const SESSION_OVERVIEW_TEXT =
  "ATW workflow system active. Use skills and agents as directed by the workflow state.";
...
class TurnContextCache {
  ...
  get(projectRoot: string, contextKey: string | null): { workflowMsg: string } {
    ...
    const workflowPath = join(projectRoot, ".atw", "workflow.md");
    let workflowMd = "";
    try {
      workflowMd = readFileSync(workflowPath, "utf-8");
    } catch {}
...
const ATW_AGENTS = new Set(["atw-implement-agent", "atw-review"]);

function detectAgentType(): AgentType {
  const blocked = process.env.PI_BLOCKED_AGENT;
  if (blocked && ATW_AGENTS.has(blocked)) {
...
export default function (pi: ExtensionAPI): void {
  ...
  const getTaskContext = (taskDir: string, root: string): string => {
    ...
    if (
      taskContextCache?.projectRoot === root &&
      taskContextCache.taskDir === taskDir &&
      taskContextCache.agentType === agentType &&
      taskContextCache.signature === signature
    ) {
  ...
  pi.on("session_start", async (_event, ctx) => {
    ...
    if (isSubAgent) {
      ...
      if (sessionContext) {
        await pi.sendMessage({
          customType: "atw-session-context",
      ...
      ctx.ui.notify("ATW workflow system available", "info");
    }
  });
  ...
  pi.on("context", async (event, ctx) => {
    ...
    const taskContextIndexes = messages
      .map((message, index) =>
        message.customType === "atw-task-context" ? index : -1,
      )
      .filter((index) => index >= 0);
    const existingTaskContext =
      taskContextIndexes.length > 0
        ? (messages[taskContextIndexes[0]]?.content ?? "")
        : "";
    const taskContextChanged =
      existingTaskContext !== currentTaskContext ||
      taskContextIndexes.length > 1;
    ...
    if (taskContextChanged) {
      ...
      projectedMessages = messages.flatMap((message) => {
        if (message.customType !== "atw-task-context") return [message];
    ...
    if (!taskContextChanged && !skipping && lastInjectionTs > lastCompactionTs)
      return;
    ...
    if (skipping) {
      ...
      const withoutBreadcrumb = projectedMessages.filter(
        (message) =>
          !(
            message.role === "custom" &&
            message.customType === "atw-workflow-state"
          ),
      );
      if (
        withoutBreadcrumb.length === projectedMessages.length &&
        !taskContextChanged
      )
        return;
      lastInjectionTs = Date.now();
    ...
    for (let i = projectedMessages.length - 1; i >= 0; i--) {
      if (
        projectedMessages[i].role === "custom" &&
        projectedMessages[i].customType === "atw-workflow-state"
      ) {
        lastInjectionTs = Date.now();
