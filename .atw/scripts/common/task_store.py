...
Provides:
    ...
    cmd_set_status     - Move a task to one of the six workflow stages
...
def cmd_create(args: argparse.Namespace) -> int:
    ...
    # which would otherwise produce MM-DD-MM-DD-slug. Only an
    ...
    # mis-stamp that feature branch as the PR target. Falls
    ...
    # Create an empty check.jsonl for sub-agent-capable platforms. It stays
    # empty until the agent curates real entries — a placeholder row would read
    # as unresolved scaffolding to `task.py validate` and to PR preflight, so
    # the curation instructions go to the console below instead of into the
    # file. Agent-less platforms (Kilo / Antigravity / Devin) skip this — they
    # load specs via the atw-before-dev skill instead of JSONL.
    #
    # implement.jsonl is deliberately not seeded: the implement sub-agent gets
    # its spec from the current ticket (`tickets.py claim` writes that path into
    # check.jsonl), so a seeded-but-empty implement manifest would only be a
    # file nobody curates. The `start` gate only refuses manifests that exist
    # with zero curated entries, so an absent one is not gated.
    created_jsonl = False
    ...
    print("  - Complex task: slice it into issues/NN-slug.md tickets before task.py start", file=sys.stderr)
    if created_jsonl:
        print(
            "  - Curate check.jsonl (created empty) as the spec/research "
            "manifest before task.py start when sub-agents need context:",
            file=sys.stderr,
        )
        print(
        ...
        print(
            "      list available specs: python3 .atw/scripts/get_context.py --mode packages",
            file=sys.stderr,
        )
    print(
        "  - Use the ATW `continue` command or phase context to decide the next step",
        file=sys.stderr,
    )
    print("", file=sys.stderr)
...
class _RenamePlan:
    ...
    # (file, line number) of old-name mentions elsewhere under .atw/
...
def _plan_reported_refs(
    ...
    """Find remaining mentions of the old task name elsewhere under .atw/.
    ...
    *are* rewritten — see ``repoint_task_in_sessions``. Listing them here as
    ...
    atw_dir = repo_root / DIR_WORKFLOW
    sessions_dir = _runtime_sessions_dir(repo_root)
    if not atw_dir.is_dir():
        return hits

    for path in sorted(atw_dir.rglob("*")):
        ...
        if any(
            ...
            for part in path.relative_to(atw_dir).parts[:-1]
        ):
            continue
        try:
...
def _validate_branch_metadata(
    ...
    rather than repaired by hand later (follow-up).
...
def cmd_archive(args: argparse.Namespace) -> int:
    ...
    # .atw/tasks/. resolve_task_dir keeps the target inside the tasks dir;
    # this narrows it further to a direct child, so an already-archived task
    # (.atw/tasks/archive/<month>/<task>) is not archived a second time.
    if not is_within_tasks_dir(task_dir, repo_root):
        print(colored(
            f"Error: refusing to archive '{task_name}': "
    ...
    if task_json_path.is_file():
        ...
        if data is None:
            ...
            # 中断重跑不该改写原始完成日；用 set-status completed 造出的中断态仍需补上
                    ...
                    if child_dir_path:
                        ...
                        if child_json.is_file():
                            ...
                            if not write_json(child_json, child_data):
                                ...
                                # parent that has left .atw/tasks/ is a
...
def _auto_commit_archive(
    ...
    """Stage ATW-owned task paths and commit after archive.
    ...
    ``git add -f .atw/`` (which would fan out to caches/backups)
    and points users at ``session_auto_commit: false``.

    Honors ``session_auto_commit`` in ``.atw/config.yaml``: when
    ...
    # chore commit. `source_rel` is included so the source-side
...
def _print_index_lock_warning(
    ...
    command keeps the same narrow scope — a blanket ``git add -A -- .atw/``
...
# The six workflow stages, in order. `create` seeds the first one.
#
# The fifth stage is "accept", not "finish": `task.py finish` is an unrelated
# command (it clears the active-task pointer), and a stage sharing that word
# would make every mention of it ambiguous.
#
# This tuple is the Python half of a two-language pair; the other half is
# `TASK_STATUS_VALUES` in `@chinhae/atw-core` (packages/core/src/task/schema.ts).
# Python cannot import TypeScript, so the copies stay pinned by a test:
# packages/cli/test/templates/task-status-parity.test.ts. Editing one side
# without the other turns that test red.
STATUS_VALUES = ("discover", "specify", "slice", "implement", "accept", "completed")


def cmd_set_status(args: argparse.Namespace) -> int:
    """Move a task to one of the six workflow stages."""
    status = args.status

    # Validated before anything is read or written: a rejected value must leave
    # task.json byte-identical. The retired values (planning / in_progress /
    # review / done) are refused here like any other typo — a set-status that
    # accepts them is the old vocabulary still in production, just undocumented.
    if status not in STATUS_VALUES:
        print(colored(f"Error: Invalid status: {status}", Colors.RED), file=sys.stderr)
        print(f"Allowed: {' '.join(STATUS_VALUES)}", file=sys.stderr)
        return 1
    ...
    print(colored(f"✓ Status: {previous} → {status}", Colors.GREEN))
    return 0
