...
Note:
    ``cmd_init_context`` no longer exists. JSONL context files
    ...
    ``.atw/workflow.md`` for the current planning artifact contract.

    Older ATW versions seeded those files with a ``{"_example": ...}``
...
# one of these extensions outside .atw/spec/, docs/docs-site, or the
...
def cmd_validate(args: argparse.Namespace) -> int:
    ...
    # was likely already merged and deleted.
...
def _is_exempt_from_code_file_warning(file_path: str, task_rel: str) -> bool:
    ...
    Exempt: spec docs (``.atw/spec/``), documentation (``docs``,
    ...
    exempt_prefixes = (".atw/spec/", "docs/", "docs-site/")
...
def _validate_jsonl(jsonl_file: Path, repo_root: Path, task_dir: Path | None = None) -> int:
    ...
    ``{"_example": ...}`` placeholder rows written by older ATW versions
    ...
    if errors == 0 and real_entries == 0:
        ...
        # would run with zero spec context. Silent-green here is how
        ...
        print(
            f"    Curate it:  python3 .atw/scripts/task.py add-context <task> {action} <path> \"<why>\""
        )
        print(
