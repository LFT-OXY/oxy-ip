"""
Safe git-add helpers for ATW-owned paths.

Why this module exists
----------------------
A real user incident: a project's `.gitignore` listed `.atw/` (company-wide
...
`git add -f .atw/` — which fan-out-included every ignored subtree
(`.atw/.backup-*/`, `.atw/worktrees/`, `.atw/.template-hashes.json`,
`.atw/.runtime/`), committing 548 files / 83474 lines of caches/backups.

Design
------
- Scripts only stage SPECIFIC product paths (journal files, index.md, the
  current task dir, the archive dir). Never the whole `.atw/` tree.
- If plain `git add <specific>` fails with "ignored by", DO NOT retry with
  ``-f``. The presence of `.atw/` in `.gitignore` is treated as user
  intent ("keep .atw/ local-only"). The script warns and skips the
...
- The warning includes a negative example: ``Do NOT use `git add -f .atw/` ...``
  so any AI rereading the log doesn't reinvent the bug.

History note: an automatic ``git add -f`` retry on the specific paths was
tried once and then removed — auto-forcing into a tree the user had
gitignored violates user intent even when the path list is narrow. The
wider-grain forbidden command stays forbidden, and the narrow-grain auto
``-f`` is gone too. Do not reintroduce either.
"""
...
# Paths under .atw/ that must NEVER be auto-staged. Listed here so the
# warning to the user can show concrete subpaths to ignore individually
# instead of ignoring the whole `.atw/` tree.
ATW_IGNORED_SUBPATHS = (
    ".atw/.backup-*",
    ".atw/worktrees/",
    ".atw/.template-hashes.json",
    ".atw/.runtime/",
    ".atw/.cache/",
)


def safe_atw_paths_to_add(
    repo_root: Path,
    ...
    Included:
      - .atw/workspace/<developer>/journal-*.md
      - .atw/workspace/<developer>/index.md
      - .atw/tasks/<task_name>/   (ONLY the current task dir when
    ...
    Excluded (intentionally — these must not be staged):
      - .atw/.backup-*, .atw/worktrees/,
        .atw/.template-hashes.json, .atw/.runtime/, .atw/.cache/

    Scope contract (see the break-loop analysis): when ``task_name`` is
...
def safe_archive_paths_to_add(
    ...
    `.atw/tasks/` subtree the old way (active tasks + archive). New
...
def safe_git_add(
    ...
    kept for signature compatibility with the earlier auto-force
    implementation but is always ``False`` — we never auto-force.
...
def print_gitignore_warning(paths: list[str]) -> None:
    ...
    ``Do NOT use `git add -f .atw/``` — agents reading the warning are
    known to invent that command, which fans out to ignored caches/backups.
    """
    print(
        "[WARN] git add failed because .atw/ paths are ignored by your .gitignore.",
        file=sys.stderr,
    )
    print(
    ...
    print(
        "[WARN] ATW manages these specific paths and they should be tracked:",
        file=sys.stderr,
    )
    if paths:
        for p in paths:
        ...
        print(
            "[WARN]   .atw/workspace/<developer>/{journal-*.md,index.md}",
            file=sys.stderr,
        )
        print(
            "[WARN]   .atw/tasks/<task-dir>/",
            file=sys.stderr,
        )
        print(
            "[WARN]   .atw/tasks/archive/",
            file=sys.stderr,
        )
    print("[WARN]", file=sys.stderr)
    print(
        "[WARN] Recommended: change your .gitignore from `.atw/` to specific",
        file=sys.stderr,
    )
    print(
    ...
    for sub in ATW_IGNORED_SUBPATHS:
        print(f"[WARN]   {sub}", file=sys.stderr)
    print("[WARN]", file=sys.stderr)
    print(
        "[WARN] Or, if you intentionally keep .atw/ local-only, set in",
        file=sys.stderr,
    )
    print(
        "[WARN] .atw/config.yaml:",
        file=sys.stderr,
    )
    print(
    ...
    print("[WARN]", file=sys.stderr)
    print(
        "[WARN] Do NOT use `git add -f .atw/` — it pulls in backups, worktrees,",
        file=sys.stderr,
    )
    print(
