...
The user-facing concept is a single "active task". ATW stores that pointer
per AI session/window under `.atw/.runtime/sessions/`; without a stable
...
# with a neighbour: an audit of all 21 platforms found 12 of the 21
...
# table; it resolves through ATW_CONTEXT_ID or its hook/plugin bridge.
_ENV_SESSION_KEYS: tuple[tuple[str, tuple[str, ...]], ...] = (
    # REAL (reported from a live run: DSH exports DSH_SESSION_ID plus
    # DSH_SHELL=1 into its managed shell).
    ...
    ("dsh", ("DSH_SESSION_ID",)),
    # REAL, undocumented (verified in a live Claude Code 2.1.221 bash
    ...
    # REAL, undocumented (verified: injected by codex-cli 0.146.0
    ...
    # REAL but HOOK-SCOPE ONLY (verified): set by Gemini's
    ...
    # REAL but HOOK-SCOPE ONLY (verified): docs.qoder.com/zh/
    ...
    # UNVERIFIED: absent from kiro.dev/docs/hooks/, but Dynatrace
    ...
    # UNVERIFIED: absent from docs.github.com/en/copilot/reference/
    ...
    # REASONED, UNVERIFIED: ZCode is closed-source and not
    ...
    # REAL by vendor design (verified): Snow's sessionIdentityEnv.ts
    # exports SNOW_SESSION_ID into hook/terminal/sub-agent children and names
    # ATW in its source header. ATW_CONTEXT_ID stays the preferred
    # override — Snow sets that too.
    ("snow", ("SNOW_SESSION_ID",)),
)
_ENV_CONVERSATION_KEYS: tuple[tuple[str, tuple[str, ...]], ...] = (
    # REAL in cursor-agent (CLI), undocumented (verified: the value
    # matches ~/.cursor/chats/<ws>/<id>). The Cursor *IDE* is unverified — a
    # forum request for it drew no staff reply. The invented
...
_ENV_TRANSCRIPT_KEYS: tuple[tuple[str, tuple[str, ...]], ...] = (
    # REAL but HOOK-SCOPE ONLY (verified): documented for Cursor hook
    # scripts; empty in the agent's own shell env.
    ("cursor", ("CURSOR_TRANSCRIPT_PATH",)),
    # UNVERIFIED — never researched. The audit noted above covered the session
...
def _detect_platform(platform_input: dict[str, Any] | None, platform: str | None) -> str:
    ...
    if platform_input:
        for key in ("_atw_platform", "atw_platform", "platform", "source"):
...
def _lookup_env_context_key(platform_name: str | None) -> str | None:
    ...
    Hooks pass `ATW_CONTEXT_ID` to subprocesses they launch, but an AI-run
...
def resolve_context_key(
    ...
    *,
    ...
    `ATW_CONTEXT_ID` is an explicit context-key override used by CLI
    scripts and subprocesses. It does not store the task itself.
    """
    if allow_environment_context:
        override = _string_value(os.environ.get("ATW_CONTEXT_ID"))
        if override:
...
def _write_json(path: Path, data: dict[str, Any]) -> bool:
    ...
    temp-file-then-rename treatment as task.json. A plain write_text
...
def _relative_task_ref(task_path: str, repo_root: Path) -> str:
    ...
    # symlinked `.atw` maps back to its in-repo form; anything
...
def resolve_active_task(
    ...
    *,
    ...
    A stale session task is returned as stale.
