#!/usr/bin/env python3
"""
ATW configuration reader.

Reads settings from .atw/config.yaml with sensible defaults.
"""
...
from .atw_config import parse_simple_yaml

# The YAML subset parser lives in atw_config.py — it imports nothing from
...
def _load_config(repo_root: Path | None = None) -> dict:
    ...
    Fail-open, matching ``atw_config.read_atw_config``: a malformed
...
def get_session_auto_commit(repo_root: Path | None = None) -> bool:
    ...
    Set ``session_auto_commit: false`` in ``.atw/config.yaml`` to skip
...
def get_codex_dispatch_mode(repo_root: Path | None = None) -> str:
    ...
    Default is ``auto``, which dispatches ATW sub-agents and uses native
...
def get_context_injection_limits(repo_root: Path | None = None) -> dict[str, int]:
    ...
    Reads the ``context_injection:`` section of ``.atw/config.yaml``:
...
def get_prompt_injection_config(repo_root: Path | None = None) -> dict[str, str]:
    ...
    Reads the ``prompt_injection:`` section of ``.atw/config.yaml``:

        prompt_injection:
          skip_keyword: "no-atw"   # "" disables the escape hatch entirely
    ...
    emit nothing for that turn. Defaults to ``"no-atw"``. A non-string
...
def get_spec_base(package: str | None = None, repo_root: Path | None = None) -> str:
    """Get the spec directory base path relative to .atw/.
