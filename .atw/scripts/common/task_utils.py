...
def is_within_tasks_dir(task_dir_abs: Path, repo_root: Path | None = None) -> bool:
    ...
    A real task lives directly at ``.atw/tasks/<name>``. This returns True
...
def resolve_task_dir(target_dir: str, repo_root: Path) -> Path | None:
    ...
    - Relative path: .atw/tasks/01-31-my-task
    ...
    land strictly under ``.atw/tasks/``; archived tasks under
    ...
    ``.atw`` that is itself a symlink into a shared store works.
    ...
    elif "/" in normalized or normalized.startswith(".atw"):
        # Relative path (contains path separator or starts with .atw)
    ...
    # `resolved`: `.atw` may be a symlink into a store outside the repo
    #, in which case `resolved` sits outside the repo even though the
...
def run_task_hooks(event: str, task_json_path: Path, repo_root: Path) -> None:
    ...
    Hooks are shell commands read from ``.atw/config.yaml`` and executed
    with ``shell=True`` from the repo root — see the trust boundary note in
    ``.atw/spec/cli/backend/script-conventions.md``.
...
if __name__ == "__main__":
    ...
    print(f"resolve_task_dir('.atw/tasks/test'): {resolve_task_dir('.atw/tasks/test', repo)}")
