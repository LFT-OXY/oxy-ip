#!/usr/bin/env python3
"""
Common path utilities for ATW workflow.
...
def get_repo_root(start_path: Path | None = None) -> Path:
    """Find the nearest directory containing .atw/ folder.
    ...
    Returns:
        Path to repository root, or current directory if no .atw/ found.
    """
    ...
    # Fallback to current directory if no .atw/ found
...
def get_developer(repo_root: Path | None = None) -> str | None:
    ...
    it, before this function is ever called):
        ...
        1. The ``ATW_DEVELOPER`` environment variable.
        2. ``.atw/.developer`` in this checkout.
        3. ``.atw/.developer`` in the main checkout, when this checkout is a
           linked git worktree.
...
def normalize_task_ref(task_ref: str) -> str:
    ...
    `.atw/tasks/03-27-my-task`, even on Windows. Absolute paths are preserved
...
def get_spec_dir(package: str | None = None, repo_root: Path | None = None) -> Path:
    ...
    Single-repo: .atw/spec
    Monorepo with package: .atw/spec/<package>
