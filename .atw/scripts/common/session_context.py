...
def _discover_child_git_repos(repo_root: Path) -> list[tuple[str, str]]:
    ...
    if overflow:
        print(
            ...
            "packages entries with path and git: true in .atw/config.yaml.",
            file=sys.stderr,
        )
        return []
    if len(found) < 2:
...
def _collect_package_git_info(
    ...
    When the ATW root is not a Git repo and no configured package repos are
...
def _fetch_atw_version_output() -> str | None:
    try:
...
def _resolve_available_update_version() -> str | None:
    output = _fetch_atw_version_output()
    if not output:
        return None
...
def get_update_hint(repo_root: Path, context_key: str | None = None) -> str | None:
    ...
    return (
        f"ATW update available: {current_version} -> {latest_version}, "
        "run atw update"
    )
...
def get_context_text(repo_root: Path | None = None) -> str:
    ...
    for t in all_tasks.values():
        if t.assignee == developer and t.status != "completed":
            progress = children_progress(t.children, all_statuses)
