...
class CLIAdapter:
    ...
    _AGENT_NAME_MAP: ClassVar[dict[Platform, dict[str, str]]] = {
        ...
        "opencode": {
            "plan": "atw-plan",  # 'plan' is built-in in OpenCode
        },
    }
    ...
    def get_agent_name(self, agent: str) -> str:
        ...
        Returns:
            Platform-specific agent name (e.g., 'atw-plan' for OpenCode)
        """
    ...
    def get_commands_path(self, project_root: Path, *parts: str) -> Path:
        ...
        Args:
            project_root: Project root directory
            *parts: Additional path parts (e.g., 'atw', 'finish-work.md')

        Returns:
            Path to commands directory or file

        Note:
            Cursor uses prefix naming: .cursor/commands/atw-<name>.md
            Antigravity uses workflow directory: .agent/workflows/<name>.md
            Devin uses workflow directory: .devin/workflows/atw-<name>.md
            Copilot uses prompt files: .github/prompts/<name>.prompt.md
            Pi uses prompt templates: .pi/prompts/atw-<name>.md
            Claude/OpenCode use subdirectory: .claude/commands/atw/<name>.md
        """
        if self.platform == "pi":
            prompts_dir = self.get_config_dir(project_root) / "prompts"
            if not parts:
                return prompts_dir
            if len(parts) >= 2 and parts[0] == "atw":
                filename = parts[-1]
                if filename.endswith(".md"):
                    filename = filename[:-3]
                return prompts_dir / f"atw-{filename}.md"
            return prompts_dir / Path(*parts)
        # OMP and Grok: flat slash commands under .{platform}/commands/atw-<name>.md
        if self.platform in ("omp", "grok"):
            ...
            if len(parts) >= 2 and parts[0] == "atw":
                ...
                if filename.endswith(".md"):
                    filename = filename[:-3]
                return commands_dir / f"atw-{filename}.md"
            return commands_dir / Path(*parts)

        # Kimi: commands are skills under .kimi-code/skills/atw-<name>/SKILL.md
        if self.platform == "kimi":
            ...
            if len(parts) >= 2 and parts[0] == "atw":
                ...
                if filename.endswith(".md"):
                    filename = filename[:-3]
                return skills_dir / f"atw-{filename}" / "SKILL.md"
        ...
        if self.platform == "devin":
            ...
            if len(parts) >= 2 and parts[0] == "atw":
                filename = parts[-1]
                return workflow_dir / f"atw-{filename}"
        ...
        if self.platform == "cursor" and len(parts) >= 2 and parts[0] == "atw":
            # Convert atw/<name>.md to atw-<name>.md
            filename = parts[-1]
            return (
                self.get_config_dir(project_root) / "commands" / f"atw-{filename}"
            )

        return self.get_config_dir(project_root) / "commands" / Path(*parts)

    def get_atw_command_path(self, name: str) -> str:
        """Get relative path to a atw command file.
        ...
        Note:
            Cursor: .cursor/commands/atw-<name>.md
            Codex: .agents/skills/atw-<name>/SKILL.md
            Kiro: .kiro/skills/atw-<name>/SKILL.md
            Gemini: .gemini/commands/atw/<name>.toml
            Antigravity: .agent/workflows/<name>.md
            Devin: .devin/workflows/atw-<name>.md
            Pi: .pi/prompts/atw-<name>.md
            Others: .{platform}/commands/atw/<name>.md
        """
        if self.platform == "cursor":
            return f".cursor/commands/atw-{name}.md"
        elif self.platform == "codex":
            # Codex skill dirs are always written with the `atw-` prefix.
            return f".agents/skills/atw-{name}/SKILL.md"
        elif self.platform == "kiro":
            return f".kiro/skills/atw-{name}/SKILL.md"
        elif self.platform == "gemini":
            return f".gemini/commands/atw/{name}.toml"
        elif self.platform == "antigravity":
            return f".agent/workflows/{name}.md"
        elif self.platform == "devin":
            return f".devin/workflows/atw-{name}.md"
        elif self.platform == "kilo":
            return f".kilocode/workflows/{name}.md"
        elif self.platform == "copilot":
            return f".github/prompts/{name}.prompt.md"
        elif self.platform == "droid":
            return f".factory/commands/atw/{name}.md"
        elif self.platform == "pi":
            return f".pi/prompts/atw-{name}.md"
        elif self.platform in ("omp", "grok"):
            return f"{self.config_dir_name}/commands/atw-{name}.md"
        elif self.platform == "kimi":
            return f".kimi-code/skills/atw-{name}/SKILL.md"
        else:
            return f"{self.config_dir_name}/commands/atw/{name}.md"
...
def detect_platform(project_root: Path) -> Platform:
    ...
    Detection order:
    1. ATW_PLATFORM environment variable (if set)
    ...
    env_platform = os.environ.get("ATW_PLATFORM", "").lower()
    # Deprecated alias: Windsurf was renamed to Devin.
    ...
    # as devin for back-compat until users migrate via `atw update --migrate`).
    if (
    ...
    # (.agents/skills/atw-* dirs) and no explicit platform config dir.
    ...
    if agents_skills.is_dir() and not _has_other_platform_dir(
        ...
        try:
            for entry in agents_skills.iterdir():
                if entry.is_dir() and entry.name.startswith("atw-"):
                    return "codex"
        except OSError:
            pass
