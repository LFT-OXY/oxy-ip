...
Usage:
    ...
    python3 task.py set-status <dir> <status>   # Move task to a workflow stage
...
def _record_start_state(
    ...
    """Move a planning-stage task to `implement` and record its branch.

    Both updates share one read/write: the status flip out of a planning stage,
    and the checked-out branch when `branch` is still empty. Recording at start
    is what keeps `branch` trustworthy at archive time — a task whose branch is
    only ever set by hand tends to reach archive with `branch: null`.

    Every stage before `implement` is a planning stage and all of them leave
    through `start`; `implement` and later keep what they have, because `start`
    is also how you resume a task after a checkout.

    This used to flip `discover` only. That was equivalent back when there was
    a single `planning` stage, but the six-stage split (C1/C2) turned it into
    discover / specify / slice — and "flip the first one only" stranded every
    sliced task: workflow.md's slice breadcrumb says "run `task.py start` to
    enter implement", `start` silently did nothing, and the per-turn hook kept
    re-issuing the slice breadcrumb forever. The complex path could not leave
    Phase 1. Found by the N8 end-to-end walk; no source-tree criterion sees it.
    ...
    absent status line looks like the task simply was past planning.
    """
    ...
    # 从 STATUS_VALUES 切出来，不另写一份名单：六阶段词表再改一次时这里跟着走。
    planning_stages = STATUS_VALUES[: STATUS_VALUES.index("implement")]
    previous_status = data.get("status")
    if previous_status in planning_stages:
        data["status"] = "implement"
        applied.append(f"✓ Status: {previous_status} → implement{label}")
...
def cmd_start(args: argparse.Namespace) -> int:
    ...
    if not full_path.is_dir():
        ...
        print("Hint: Use task name (e.g., 'my-task') or full path (e.g., '.atw/tasks/01-31-my-task')")
        return 1

    # Context-manifest gate: a seeded-but-uncurated implement/check
    ...
    if not getattr(args, "allow_empty_context", False):
        ...
        if empty_manifests:
            ...
            print("Review sub-agents (atw-review) would run with zero spec context.")
            print(f"  Curate:  python3 .atw/scripts/task.py add-context {task_input} implement <path> \"<why>\"")
            print(f"  Verify:  python3 .atw/scripts/task.py validate {task_input}")
    ...
    if not resolve_context_key():
        ...
        # Hook didn't inject ATW_CONTEXT_ID (common on Windows + Claude Code,
        ...
        print(colored(
            ...
            "or set ATW_CONTEXT_ID before running task.py start.",
            Colors.YELLOW,
        ))

        # Still flip task.json status: discover → implement so downstream phases proceed.
...
def _display_status(t, all_statuses: dict) -> str:
    ...
    A parent task's stored status stays "discover" until someone runs
    ...
    list. Show "active" instead when at least one child is
    past "discover"; the stored status.json value is left untouched.
    """
    if t.status == "discover" and t.children:
        child_in_flight = any(
            all_statuses.get(c) not in (None, "discover") for c in t.children
        )
        if child_in_flight:
            return "active"
    return t.status
...
def show_usage() -> None:
    """Show usage help."""
    # Interpolated rather than spelled out: a hand-copied stage list drifts from
    # the real one, and `--status <stale value>` then silently matches nothing.
    statuses = " | ".join(STATUS_VALUES)
    print(f"""Task Management Script
...
List options:
  ...
  --status, -s <s>     Filter by status ({statuses})
...
Examples:
  ...
  python3 task.py create "Child task" --description "Session cookie handling" --slug child --parent .atw/tasks/01-21-parent
  python3 task.py add-context <dir> implement .atw/spec/cli/backend/auth.md "Auth guidelines"
  python3 task.py set-branch <dir> task/add-login
  python3 task.py set-status add-login slice
  python3 task.py start .atw/tasks/01-21-add-login
  ...
  python3 task.py list --mine --status implement     # List my in-flight tasks
""")
...
def main() -> int:
    """CLI entry point."""
    # Deprecation guard: `init-context` no longer exists.
    # Detect early so argparse doesn't mask the real reason with a generic
    # "invalid choice" error.
    if len(sys.argv) >= 2 and sys.argv[1] == "init-context":
        print(
            colored(
                "Error: `task.py init-context` has been removed.",
                Colors.RED,
            ),
            file=sys.stderr,
        )
        print(
            "check.jsonl is now seeded on `task.py create` for sub-agent-capable",
            file=sys.stderr,
        )
        print(
            "platforms and curated by the AI when sub-agents need spec context.",
            file=sys.stderr,
        )
        print("See .atw/workflow.md planning artifact guidance or run:", file=sys.stderr)
        print(
            "  python3 ./.atw/scripts/get_context.py --mode phase --step 1",
            file=sys.stderr,
        )
        print(
    ...
    # set-status
    # `choices` is deliberately not used: argparse would exit 2 with a bare
    # "invalid choice", while cmd_set_status names the rejected value and lists
    # the allowed ones. Both refuse, only one tells you what to type instead.
    p_setstatus = subparsers.add_parser("set-status", help="Move task to a workflow stage")
    p_setstatus.add_argument("dir", help="Task directory")
    p_setstatus.add_argument("status", help=f"Workflow stage ({' | '.join(STATUS_VALUES)})")
