...
MARKER_VERSION = 2
LEGACY_MARKER_RE = re.compile(r"^<!-- atw-session: fp=([0-9a-f]{16}) -->$")
...
def warn_if_parallel_worktree(repo_root: Path) -> None:
    ...
    (not the main tree) with `session_auto_commit` enabled (quick-fix tier).
    """
    ...
    print(
        colored(
            ...
            "not index.md). See .atw/spec/cli/backend/directory-structure.md "
...
def resolve_next_session(repo_root: Path, dev_dir: Path, index_file: Path) -> int:
    ...
    each record before merging claim the same number (observed twice in
    practice). Taking the union with every recorded ref — the default
...
def content_at_head(repo_root: Path, path: Path) -> str | None:
    ...
    ATW root is the nearest directory holding `.atw/`, which can sit
...
def _auto_commit_workspace(repo_root: Path) -> str:
    """Stage ATW-owned workspace + current-task paths and commit.
    ...
    via ``get_current_task``). We never `git add` the whole `.atw/` tree
    or iterate over all active task dirs (parallel-window dirty task
    ...
    Honors ``session_auto_commit`` in ``.atw/config.yaml``: when set to
    ...
    ``COMMIT_FAILED``. ``COMMIT_BLOCKED`` is the gitignored-`.atw/` case:
    ...
    # is ``.atw/tasks/<name>`` (or under archive/) — pass the bare name.
    current = get_current_task(repo_root)
    if current:
        task_name = Path(current).name
        paths = safe_atw_paths_to_add(repo_root, task_name=task_name)
    else:
        # Current task unknown (0 or >=2 parallel sessions — exactly the
        # parallel-window case this guard is about). Do NOT fall back to the wide
        ...
        paths = [
            p
            for p in safe_atw_paths_to_add(repo_root, task_name=None)
    ...
    # the chore commit. The pathspec keeps their staged work untouched.
