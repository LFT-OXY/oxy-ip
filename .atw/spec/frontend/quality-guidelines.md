...
目录布局验收不能只检查“不溢出”：A 清单须通过实际 DOM 的 `getBoundingClientRect()` 比较表头与各行对应列起点（末列用右边缘）、开合前后入口相对本行的位置及提示全宽（误差不超过 1 CSS 像素）。覆盖全收起、首项展开及全部展开，以及 `md` 断点、窄屏、中英文、深浅主题；不再要求旧卡片布局或不同行等高。批量设置 `details.open` 只证明布局，另用鼠标、Enter、Space、Tab 验证独立操作与焦点，并查看截图；数值通过不代替用户视觉验收。
