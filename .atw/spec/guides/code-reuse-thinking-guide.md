...
**Real example**: `atw update` had a manual `files.set()` list for 11 scripts that `getAllScripts()` already tracked. Fix: replaced the manual list with a `for..of getAllScripts()` loop. See the `update.ts` refactor.

---

## Template File Registration (ATW-specific)

When adding new files to `src/templates/atw/scripts/`:

**Single registration point**: `src/templates/atw/index.ts`
...
**Why this matters**: Without registration in `getAllScripts()`, `atw update` won't sync the file to user projects. Bug fixes and features won't propagate.

**History**: `update.ts` once had its own hand-maintained file list that frequently fell out of sync with `getAllScripts()`. This caused 11 Python files to be silently skipped during `atw update`. The fix was to eliminate the duplicate list and use `getAllScripts()` as the single source of truth.

### Quick Checklist for New Scripts

```bash
# After adding a new .py file, verify it's in getAllScripts():
grep -l "newFileName" src/templates/atw/index.ts  # Should match
```

### Template Sync Convention

`.atw/scripts/` (dogfooded) and `packages/cli/src/templates/atw/scripts/` (template) must stay identical. After editing `.atw/scripts/`, always sync:

```bash
rsync -av --delete --exclude='__pycache__' .atw/scripts/ packages/cli/src/templates/atw/scripts/
```

**Gotcha**: Running rsync with wrong source/destination paths can create nested garbage directories (e.g., `.atw/scripts/packages/cli/...`). Always double-check paths before running.
