...
// ── Constants ─────────────────────────────────────────────────────────
// Agent names mirror `AGENTS_ALL` in the shared sub-agent context hook,
// which is the authority. Bare keys cover the channel-side unprefixed form
// (C34: `.atw/agents/<name>.md`); jsonl file names are unchanged (C6/C7).
const ATW_AGENT_JSONL: Record<string, string> = {
  "atw-implement-agent": "implement.jsonl",
  "implement-agent": "implement.jsonl",
  "atw-review": "check.jsonl",
  review: "check.jsonl",
};
...
function cmdHasAtwCtx(cmd: string) {
  const t = cmd.trimStart();
  return (
    /^export\s+ATW_CONTEXT_ID=/.test(t) ||
    /^ATW_CONTEXT_ID=/.test(t) ||
    /^env\s+.*ATW_CONTEXT_ID=/.test(t)
  );
}
...
function resolvePiCli(): { command: string; args: string[] } {
  const envCli = str(process.env.ATW_PI_CLI_JS);
  if (envCli) {
    const p = resolve(envCli);
    if (!exists(p)) throw new Error(`ATW_PI_CLI_JS missing: ${p}`);
 ...
 * `.atw/config.yaml`. Not a general YAML parser — mirrors
...
function readContextInjectionLimits(repoRoot: string): ContextInjectionLimits {
  ...
  const text = readText(join(repoRoot, ".atw", "config.yaml"));
...
function binaryNotice(path: string, size: number, reason: string): string {
  return `[ATW: not inlined (binary file) — ${path} (${size} bytes): ${reason}]`;
}
function indexNotice(path: string, size: number, reason: string): string {
  return `[ATW: not inlined (total context limit reached) — ${path} (${size} bytes): ${reason}]`;
}
...
function findRoot(start: string): string {
  let c = resolve(start);
  while (true) {
    // Only a directory with `.atw/` is an ATW project root. A bare
    // `.pi` can be pi's global config (`~/.pi`) or an unrelated project, so
    // accepting it here made root resolution stop too early (e.g. on `~`).
    // Also reject a regular file named `.atw` — the marker must be a
    // directory.
    const marker = join(c, ".atw");
 ...
 * before that fix can still hold one, and `atw update` does not rewrite
...
function readTaskDir(root: string, key: string | null): string | null {
  ...
  try {
    const ctx = JSON.parse(
      readText(join(root, ".atw", ".runtime", "sessions", `${key}.json`)),
    ) as JsonObject;
    let ref = str(ctx.current_task);
    if (!ref) return null;
    ref = ref;
    ref = ref.replace(/\\/g, "/").replace(/^\.\//, "");
    if (ref.startsWith("tasks/")) ref = `.atw/${ref}`;
    const candidate = ref.startsWith(".atw/")
      ? join(root, ref)
      : isAbsolute(ref)
        ? ref
        : join(root, ".atw", "tasks", ref);
...
function workflowBreadcrumb(root: string, key: string | null): string {
  const wf = readText(join(root, ".atw", "workflow.md"));
  if (!wf) return "";
...
function runContextScript(
  ...
  const script = join(root, ".atw", "scripts", "get_context.py");
  if (!exists(script)) return "";
  try {
    const py = process.platform === "win32" ? "python" : "python3";
    const result = spawnSync(py, [script, ...args], {
      cwd: root,
      env: key ? { ...process.env, ATW_CONTEXT_ID: key } : process.env,
      encoding: "utf-8",
...
function buildContext(root: string, agent: string, key: string | null): string {
  const dir = readTaskDir(root, key);
  if (!dir) return "No active ATW task found. Read .atw/ before proceeding.";
  ...
  const jsonlName = ATW_AGENT_JSONL[agent] ?? "";
  ...
  // 2. Task requirements. `design.md` / `implement.md` are not materialized
  //    (D32/C23): nothing writes them, and a reader that still reaches for
  //    them is what keeps the cut from landing.
  ...
  // prd already carries its own "=== path (label) ===" header
...
function normalizeAgent(agent: string | undefined): string {
  const name = agent ?? "atw-implement-agent";
  return name.startsWith("atw-") ? name : `atw-${name}`;
}

function isAtwAgent(root: string, agent: string): boolean {
...
function runPi(
  ...
  return new Promise((resolve) => {
    ...
    const childEnv = {
      ...process.env,
      ATW_SUBAGENT_CHILD: "1",
      ...(key ? { ATW_CONTEXT_ID: key } : {}),
    };
...
// ── Extension ──────────────────────────────────────────────────────────
export default function atwExtension(pi: {
  ...
  if (process.env.ATW_SUBAGENT_CHILD === "1") return;
  ...
  // would otherwise stay B's — and its <atw-task-context-update>
  ...
  pi.registerTool?.({
    name: "atw_subagent",
    label: "ATW Subagent",
    description: "Run an ATW project sub-agent with active task context.",
    promptSnippet:
      'Sub-agent dispatch protocol (ATW): your dispatch prompt MUST start with one line "Active task: <task path from `task.py current`>" before any other instructions.',
    promptGuidelines: [
    ...
    parameters: {
      ...
      properties: {
        agent: {
          type: "string",
          description: "Agent name, such as atw-implement-agent or atw-review.",
        },
        prompt: {
          type: "string",
    ...
    execute: async (
      ...
      const agentName = normalizeAgent(input.agent);
      if (!isAtwAgent(root, agentName)) {
        return {
          content: [
            {
              type: "text",
              text:
                "`atw_subagent` is only for ATW workflow agents with a definition file in .pi/agents/.\n\n" +
                ...
                `- Create .pi/agents/${agentName}.md for your custom ATW agent\n` +
                "- Install a community subagent package: pi install -l npm:@tintinweb/pi-subagents",
            },
          ],
          details: { agent: agentName, error: "not a atw workflow agent" },
        };
      }
      const mode = input.mode ?? "single";
    ...
    renderResult: (
      ...
      const isProgress =
        ...
        result.details.kind === "atw-subagent-progress";
  ...
  pi.on?.("tool_call", (event, ctx) => {
    ...
    if (
      ...
      !cmdHasAtwCtx(ev.input.command)
    )
      ev.input.command = `export ATW_CONTEXT_ID=${shellQuote(k)}; ${ev.input.command}`;
  });
  ...
  pi.on?.("tool_result", (event) => {
    ...
    if (
      ev.toolName === "atw_subagent" &&
      isObj(ev.details) &&
      ev.details.kind === "atw-subagent-progress" &&
  ...
  pi.on?.("before_agent_start", (event, ctx) => {
    ...
    const freshTaskCtx = buildContext(root, "atw-implement-agent", k);
    ...
    if (freshTaskCtx !== lastSentTaskCtx.get(key) || switchedRoot) {
      lastSentTaskCtx.set(key, freshTaskCtx);
      updates.push(
        "<atw-task-context-update>\nTask context changed on disk. This supersedes the ATW Task Context in the system prompt.\n\n" +
          freshTaskCtx +
          "\n</atw-task-context-update>",
      );
    }
