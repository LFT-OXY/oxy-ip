...
`.atw/spec/` is the user's project-specific engineering spec library. ATW is not about making AI memorize conventions; it injects relevant specs or requires the AI to read them at the right time.
...
`.atw/config.yaml` can declare packages:
...
```jsonl
{"file": ".atw/spec/cli/backend/index.md", "reason": "CLI backend conventions"}
{"file": ".atw/spec/cli/unit-test/conventions.md", "reason": "Test expectations"}
```
...
When the AI learns a new rule during implementation or debugging, it should update `.atw/spec/` rather than only summarizing it in chat.

## Local Customization Points

| Need                                              | Edit location                                                                                                                       |
| ------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| Add a new spec layer                              | `.atw/spec/<package>/<layer>/index.md` and corresponding guideline files.                                                           |
| Change monorepo spec mapping                      | `packages` / `default_package` / `spec_scope` in `.atw/config.yaml`.                                                                |
...
| Change when specs should be updated               | The `implement` chain in `.atw/workflow.md` (spec update runs before the commit, inside the chain) and the `atw-update-spec` skill. |

## Boundaries

`.atw/spec/` is the user's project specification, not a permanent copy of ATW built-in templates. The AI should encourage the user to update it according to the actual project code instead of treating ATW default templates as immutable documents.
