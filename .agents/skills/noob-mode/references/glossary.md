...
A product-neutral meaning reference for technical terms used by AI agents and software tools. Entries are written in English as semantic source material; explain the same meaning and analogy in Chinese when Chinese is the active language.
...
**Plain-language meaning:** A project folder that remembers every change ever made to its files.
...
**Plain-language meaning:** A separate copy of your project where you can try changes without affecting the original.
...
**Plain-language meaning:** Saving a snapshot of your work with a note about what you changed.
...
**Plain-language meaning:** Combining changes from one copy back into the original.
...
**Plain-language meaning:** A formal request saying "I made these changes — can someone review them before we make them official?"
...
**Plain-language meaning:** Downloading a complete copy of a project from a server to your computer.
...
**Plain-language meaning:** Making your own personal copy of someone else's project on the server.
...
**Plain-language meaning:** A comparison showing exactly what changed between two versions — what was added, removed, or modified.
...
**Plain-language meaning:** A waiting area where you place files before saving a snapshot. Like a "to be committed" pile.
...
**Plain-language meaning:** The copy of your project that lives on a server (like GitHub), as opposed to the copy on your computer.
...
**Plain-language meaning:** Uploading your saved changes from your computer to the shared server.
...
**Plain-language meaning:** Downloading the latest changes from the shared server to your computer.
...
**Plain-language meaning:** Switching to a different branch or version of your project.
...
**Plain-language meaning:** When two people changed the same part of the same file, and the computer can't automatically figure out which version to keep.
...
**Plain-language meaning:** Temporarily saving your current work-in-progress so you can switch to something else, then come back to it later.
...
**Plain-language meaning:** The version of the project you're currently looking at / working on.
...
**Plain-language meaning:** A permanent label attached to a specific version, usually marking a release or milestone.
...
**Plain-language meaning:** The text-based control panel for your computer. You type commands instead of clicking buttons.
...
**Plain-language meaning:** The program that runs inside your terminal and interprets the commands you type.
...
**Plain-language meaning:** A program you interact with by typing commands instead of clicking a visual interface.
...
**Plain-language meaning:** A folder on your computer.
...
**Plain-language meaning:** The address of a file or folder on your computer, showing every folder you'd pass through to get to it.
...
**Plain-language meaning:** The very top-level folder on your computer — every other folder lives inside it.
...
**Plain-language meaning:** Your personal folder on the computer. On a Mac, it's `/Users/yourname`.
...
**Plain-language meaning:** A setting stored on your computer that programs can read. Like a sticky note on your monitor that apps can see.
...
**Plain-language meaning:** Sends the output of one command into another command, like an assembly line.
...
**Plain-language meaning:** Sends output to a file instead of showing it on screen. `>` replaces the file; `>>` adds to it.
...
**Plain-language meaning:** Rules about who can read, edit, or run a file. Shown as codes like `rwx` or numbers like `755`.
...
**Plain-language meaning:** A shortcut that points to another file or folder. Not a copy — just a pointer.
...
**Plain-language meaning:** Two channels for program output. stdout is normal output; stderr is error messages.
...
**Plain-language meaning:** A file containing a list of commands that run automatically, one after another.
...
**Plain-language meaning:** A way for two programs to talk to each other, following agreed-upon rules.
...
**Plain-language meaning:** A specific URL where an API accepts requests, like a specific phone extension at a company.
...
**Plain-language meaning:** A computer (or program) that provides a service when you ask for it. It waits for requests and responds.
...
**Plain-language meaning:** The program that sends requests to a server. Your web browser is a client.
...
**Plain-language meaning:** An organized collection of data that programs can search, add to, update, and delete from.
...
**Plain-language meaning:** A pre-built tool or library that a project needs in order to work.
...
**Plain-language meaning:** A bundle of code that someone else wrote, packaged up for others to use.
...
**Plain-language meaning:** A self-contained piece of code that handles one specific thing.
...
**Plain-language meaning:** A pre-built foundation that gives you a structure to build on, with rules about how to organize your code.
...
**Plain-language meaning:** Converting source code into something that can actually run.
...
**Plain-language meaning:** Translating code from the language humans write in to the language computers understand.
...
**Plain-language meaning:** A tool that checks your code for common mistakes, style issues, and potential problems — without running it.
...
**Plain-language meaning:** Code that automatically checks whether other code works correctly.
...
**Plain-language meaning:** The environment where code actually runs. Also refers to the time period when code is actively running.
...
**Plain-language meaning:** Taking finished code and putting it somewhere people can use it (a server, a website, an app store).
...
**Plain-language meaning:** A web address. The text you type in a browser's address bar.
...
**Plain-language meaning:** The language that web browsers and servers use to talk to each other. HTTPS is the secure (encrypted) version.
...
**Plain-language meaning:** A standard format for structuring data that's easy for both humans and computers to read.
...
**Plain-language meaning:** A digital key or pass that proves your identity, used instead of typing your password every time.
...
**Plain-language meaning:** A number that a web server sends back to tell you whether your request worked. Common ones:
...
**Plain-language meaning:** A way of referring to your own computer as if it were a web server. Used for testing.
...
**Plain-language meaning:** A numbered channel on a computer. Different services use different ports, like different TV channels.
...
**Plain-language meaning:** A common style for building web APIs, where you use standard web addresses and actions (GET, POST, etc.) to interact with data.
...
**Plain-language meaning:** An add-on that gives an AI agent access to external tools or information.
**Analogy:** Installing a new app on your phone that adds a capability it didn't have before.
**Example in context:** "Configure an MCP server" = "Connect the agent to an additional tool or information source."

### Tool Call

**Plain-language meaning:** When an AI agent uses one of its available abilities, such as reading a file, running a command, or searching the web.
...
**Plain-language meaning:** The moment when an agent or its host application pauses and asks for permission before an action.
...
**Plain-language meaning:** The amount of conversation and supporting information an AI model can use at one time. When it fills up, older material may be summarized or omitted.
...
**Plain-language meaning:** The AI system that powers an agent. Different models, such as GPT and Gemini, have different strengths.
...
**Plain-language meaning:** The unit of text that AI models process. Roughly, 1 token ≈ ¾ of a word.
...
**Plain-language meaning:** A set of specialized instructions or resources an agent can use for a particular type of task.
...
**Plain-language meaning:** A third-party add-on that extends what an agent or its host application can do.
**Analogy:** A browser extension — someone else built it, and you install it to add features.
**Example in context:** "Install a plugin" = "Add a third-party capability to the agent or application."

### Session

**Plain-language meaning:** One continuous interaction with an agent, from when it starts until it ends or is resumed later.
...
**Plain-language meaning:** Instructions that tell an agent your preferences, rules, and style requirements.
**Analogy:** A brief you give a new associate: "Here's how I like my memos formatted, and here's what to prioritize."
**Example in context:** "Use custom instructions" = "Apply a specific set of behavior rules to the agent."
