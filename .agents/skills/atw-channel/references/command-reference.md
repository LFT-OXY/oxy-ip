...
Authoritative current command reference for `atw channel` subcommands,
...
```bash
atw channel create <name>
  [--scope project|global]                # default: project
  [--type chat|forum]                     # default: chat
  [--task <path>]                         # associated ATW task dir
  [--project <slug>]
  [--labels a,b,c]
...
There is **no `--tag` flag** anywhere in the channel CLI; `--kind` is
...
- `--kind` is the only event-type filter, and it is constrained to the
  atw-emitted whitelist (`CHANNEL_EVENT_KINDS` in
...
- Use `atw channel interrupt` (the command) only when you actually want
...
```bash
atw channel send T --as A --stdin < /tmp/message.md
atw channel send T --as A --text-file /tmp/message.md
```
...
```bash
atw channel interrupt <name> [text]
...
```bash
atw channel spawn <name>
  [--scope project|global]
  [--agent <agent-name>]                  # loads .atw/agents/<name>.md
  ...
  [--jsonl <path>] ...                    # ATW manifest, repeatable
  [--by <agent>]                          # spawn-event author
                                          # default: ATW_CHANNEL_AS env or 'main'
...
- OOM-guard precedence: CLI flag → env var
  (`ATW_CHANNEL_WORKER_IDLE_TIMEOUT`,
  `ATW_CHANNEL_MAX_LIVE_WORKERS`) →
  `.atw/config.yaml#channel.worker_guard` → built-in defaults.

### `run [name]`

```bash
atw channel run [name?]
  [--agent <name>]
...
```bash
atw channel thread <name> <thread-key>
  [--scope project|global]
  [--raw]

atw channel thread rename <name> <old-thread> <new-thread>
  --as <agent>                            # REQUIRED
  [--scope project|global]
```
...
atw channel context delete <name>
  [--as <agent>]                          # default: main
  [--scope project|global]
  [--thread <key>]
  [--file <abs-path>] ...
...
```bash
atw channel title set <name>
  --title <text>                          # REQUIRED
  [--as <agent>]                          # default: main
  [--scope project|global]

atw channel title clear <name>
  [--as <agent>]                          # default: main
  [--scope project|global]
```
