...
Use a single agent as the default execution model. The agent may create ATW tasks for traceability, but the skill should not require a specific platform, CLI, or parallel worker model.
...
When an ATW task is useful, write a concise PRD with these sections:

```markdown
# Fill <package-or-layer> ATW Specs

## Goal

Write project-specific `.atw/spec/` guidance for <scope>.
