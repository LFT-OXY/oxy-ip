...
`atw channel messages <channel>` renders a compact, human-readable view:
...
atw channel messages <channel> --kind done --last 10
atw channel messages <channel> --kind error --last 10

# Raw (diagnostic view) — one JSON per line
atw channel messages <channel> --raw --kind progress --last 20
atw channel messages <channel> --raw --last 50
```
...
```bash
atw channel messages <channel> --raw --kind progress --last 80 \
...
Symptom: `atw channel list` shows the worker as running, but no new
...
1. **Locate the channel files.** Use `list --all --all-projects` if you are
   ...
   ```bash
   atw channel list --all --all-projects
   CHAN=~/.atw/channels/<bucket>/<channel>
   ```
...
2. **Confirm the supervisor and worker PIDs are alive.**
   ...
   `atw channel kill <name> --as <worker> --force`.
...
```bash
atw channel wait T --as main --from check --kind done --timeout 15m
atw channel wait T --as main --from check,check-cx --kind done --all --timeout 15m
atw channel wait T --as worker --tag interrupt --timeout 1h
atw channel wait T --as main --thread release-note --action status --timeout 10m
```
...
atw channel forum list <channel>

# Inspect one thread end-to-end
atw channel thread show <channel> <thread>

# Replay messages for a thread (supports --raw, --kind, --last)
atw channel messages <channel> --thread <thread> --raw --last 100

# What a specific worker still has pending
atw channel context <channel> --as <worker>
```
...
| Symptom                          | Cause                                 | Fix                                                             |
| -------------------------------- | ------------------------------------- | --------------------------------------------------------------- |
| `atw: command not found`         | CLI not installed globally            | `npm install -g @chinhae/atw-cli`                               |
...
| ghost worker in list             | supervisor died without cleanup       | `atw channel kill <name> --as <worker> --force`                 |
