---
name: atw-spec-bootstrap
description: "Bootstrap project-specific ATW coding specs with a platform-neutral single-agent workflow. Use when creating or refreshing .atw/spec guidelines, analyzing a codebase with GitNexus, ABCoder, or source inspection, decomposing package/layer spec work, and writing real codebase-backed spec docs without placeholder text."
---

# ATW Spec Bootstrap

Use this skill to create or refresh `.atw/spec/` guidelines from the real codebase. One capable agent owns the full loop: analyze the repository, choose the spec boundaries, write the docs, and verify the result. The workflow does not depend on a specific host, CLI, or agent brand.

## Workflow

1. Confirm ATW is initialized and inspect the current `.atw/spec/` tree.
...
| Writing high-signal ATW spec files        | [references/spec-writing.md](references/spec-writing.md)               |
...
- Do not leave placeholder text, empty headings, or copied boilerplate in `.atw/spec/`.

## Done Criteria

- `.atw/spec/` describes the project as it exists now.
