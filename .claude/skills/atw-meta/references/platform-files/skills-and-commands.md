...
Skills and commands are textual entry points for user interaction with ATW. Different platforms use different names, but their core purpose is the same: tell the AI how to enter the ATW flow when the user expresses a certain intent.
...
ATW workflow skills usually share one semantic set: brainstorm, before-dev, update-spec. Multi-file built-in skills such as `atw-meta` use layered references.
...
| Kimi Code      | `.agents/skills/`, `.kimi-code/skills/` (commands delivered as `/skill:atw-*` skills) |
...
They should not store task state; task state belongs in `.atw/tasks/` and `.atw/.runtime/`.
...
1. **Keep entry files short; references carry long content**. This matters especially for multi-file skills like `atw-meta`.
...
4. **Put project-specific capabilities in local skills**. Do not put team-private flows into public `atw-meta`.

If the user only wants local AI to know one more project rule, usually create a project-local skill or update `.atw/spec/` instead of changing an ATW built-in workflow skill.
