...
Use this skill only after task-creation consent has been given and the user is ready to enter ATW's `discover` stage.

If no task exists yet, create one:

```bash
TASK_DIR=$(python3 ./.atw/scripts/task.py create "<short task title>" --description "<one-line summary>" --slug <slug>)
```
...
2. Inspect available evidence before asking questions:
   ...
   - related ATW tasks, research files, and session history when present
...
6. When no user-owned decision remains, fold the technical design into `prd.md`; execution order for complex tasks comes from slicing into tickets, not from a planning doc.
...
Lightweight tasks may skip slicing; they may not skip evidence inspection, requirement convergence, final review, or fresh implementation approval.
...
Technical design goes into `prd.md` under Technical Notes, or into `.atw/spec/` when it outlives the task. There is no separate design doc:
...
Execution planning goes into tickets under `issues/`, written when the task is sliced. There is no separate plan doc:

- ordered implementation checklist
- validation commands
- risky files or rollback points

Lightweight tasks may have only `prd.md`. Work that spans more than one session is sliced into tickets before `task.py start`.

Tickets are not a replacement for the JSONL context manifest. On sub-agent-dispatch workflows, `check.jsonl` must contain at least one real spec/research entry before `task.py start`; an empty manifest, or one holding only a legacy `_example` placeholder row, does not count. Inline workflows skip this JSONL gate because Phase 2 loads context through `atw-before-dev`.
...
- Blocking open questions are empty.
- Work that spans more than one session is sliced into tickets under `issues/`.
- Sub-agent-dispatch tasks have real curated entries in `check.jsonl`; a seed-only manifest is not ready.
