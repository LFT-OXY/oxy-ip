# `atw mem` CLI Reference

Full flag reference for the five subcommands. Pin this as the authoritative source — `atw mem help` prints the same content at runtime, so anything here that drifts is a bug.
...
| --------------------------------------------- | ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `--platform claude\|codex\|opencode\|pi\|all` | all               | Default `all`. OpenCode sessions are read, but `--phase` does not apply to them — see "Caveats" below.                                                 |
...
| `--phase brainstorm\|implement\|all`          | extract           | Slice session by ATW task boundaries. `brainstorm` = `[task.py create, task.py start)`. `implement` = turns outside brainstorm windows. Default `all`. |
...
- **OpenCode does not support `--phase` slicing.** `list`, `search`, and `context` all read OpenCode sessions from its SQLite session store, but the adapter has no native phase-boundary detection. A `--phase brainstorm` or `--phase implement` request against OpenCode returns the _full_ dialogue plus an `opencode-phase-unsupported` warning, so report those results as "full session" rather than as brainstorm or implement turns.
...
Run `atw mem help` in the user's shell. The runtime help is authoritative and will be ahead of this reference during fast-moving beta releases.
