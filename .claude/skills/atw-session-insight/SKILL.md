...
- The relevant context is already in the current turn, `prd.md`, recent `git log`, or the open files. `mem` is for stuff that has fallen out of immediate reach.
...
- You are in a sub-agent (`atw-implement-agent` / `atw-review`) whose dispatch prompt already includes the curated `implement.jsonl` / `check.jsonl` context. Adding `mem` on top usually just clutters.
...
- **Update `<task>/prd.md`** if `mem` surfaced a load-bearing decision that should have been written down but wasn't. Surface the proposed edit to the user first.
...
- **Update `.atw/spec/`** if the finding is a project-wide convention or gotcha that would help future tasks. Run the `atw-update-spec` skill for that — `session-insight` ends at the discovery.
...
ATW does not prescribe a single destination. Forcing every recall into a fixed file makes the file grow into noise. Let the situation decide.
...
atw mem search "<keyword>"

# Dump dialogue from one session, optionally filtered by phase or keyword.
atw mem extract <session-id> --phase brainstorm
atw mem extract <session-id> --grep "<keyword>"

# Drill into a session: top-N hit turns + surrounding context.
atw mem context <session-id> --turns 3 --around 2

# When you do not know the session id yet, start with list + filter.
atw mem list --cwd <project-path>
atw mem projects   # → list active project cwds, then narrow
```
...
- This skill does not replace `atw-update-spec` (which is the right tool for promoting a finding into project-wide guidance) or the platform-native task / spec workflow.
