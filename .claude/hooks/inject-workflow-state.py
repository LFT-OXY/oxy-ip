#!/usr/bin/env python3
"""ATW per-turn breadcrumb hook (UserPromptSubmit / BeforeAgent equivalent).

Runs on every user prompt. Resolves the active task through ATW'
...
Silent exit 0 case (no output):
  - No .atw/ directory found (not an ATW project)
...
CODEX_NO_TASK_BOOTSTRAP_NOTICE = """<atw-bootstrap>
If you have not already loaded ATW context this session, read the `atw-start` skill once.
</atw-bootstrap>"""


# ---------------------------------------------------------------------------
# CWD-robust ATW root discovery (fixes hook-path-robustness for this hook)
# ---------------------------------------------------------------------------

def find_atw_root(start: Path) -> Optional[Path]:
    """Walk up from start to find directory containing .atw/.

    Handles CWD drift: subdirectory launches, monorepo packages, etc.
    Returns None if no .atw/ found (silent no-op).
    """
    cur = start.resolve()
...
# (so "in-review" / "blocked-by-team" work alongside "task_error").
_TAG_RE = re.compile(
...
def _read_atw_config(root: Path) -> dict:
    """Load .atw/config.yaml via the bundled atw_config helper.

    The helper lives in .atw/scripts/common; the hook lives outside the
    ...
    if str(scripts_dir) not in sys.path:
        sys.path.insert(0, str(scripts_dir))
    try:
        from common.atw_config import read_atw_config  # type: ignore[import-not-found]
    except Exception:
        return {}
    try:
        return read_atw_config(root)
    except Exception:
        return {}


DEFAULT_PROMPT_INJECTION_SKIP_KEYWORD = "no-atw"


def _resolve_skip_keyword(config: dict) -> str:
    """Read `prompt_injection.skip_keyword` from parsed .atw/config.yaml.

    Mirrors `common.config.get_prompt_injection_config()`. Defaults to
    "no-atw"; "" disables the escape hatch entirely. A non-string value
...
def prompt_has_skip_keyword(prompt: str, keyword: str) -> bool:
    ...
    Hyphen counts as a word char so "no-atwx" / "xno-atw" /
    "foo-no-atw" don't match, but punctuation/whitespace boundaries do.
...
def _resolve_codex_dispatch_mode(config: dict) -> str:
    """Normalize `codex.dispatch_mode` from .atw/config.yaml to "auto" or "inline".
...
def _codex_mode_banner(config: dict) -> str:
    ...
    Reads `codex.dispatch_mode` from .atw/config.yaml; defaults to
    `auto`, which dispatches ATW sub-agents using native Codex context
...
def main() -> int:
    ...
    root = find_atw_root(cwd)
    if root is None:
        return 0  # not an ATW project

    config = _read_atw_config(root)
    ...
    if task is None:
        ...
        # atw-brainstorm + task.py create when user describes real work.
