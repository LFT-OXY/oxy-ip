...
On the first visible assistant reply in this session, briefly acknowledge that ATW SessionStart context loaded."""
...
3. If neither provides a language, output the language-neutral fallback exactly: `ATW SessionStart ✓`.
...
def _build_first_reply_notice(update_hint: str | None) -> str:
    ...
    return (
        ...
        f"Also relay this ATW maintenance notice on its own line in that same reply: {update_hint}\n"
...
def should_skip_injection() -> bool:
    """Check if any platform's non-interactive flag is set, or if ATW
    hooks are explicitly disabled via ATW_HOOKS=0 / ATW_DISABLE_HOOKS=1.
    """
    if os.environ.get("ATW_HOOKS") == "0":
        return True
    if os.environ.get("ATW_DISABLE_HOOKS") == "1":
        return True
...
def _resolve_context_key(atw_dir: Path, input_data: dict) -> str | None:
    scripts_dir = atw_dir / "scripts"
...
def _persist_context_key_for_bash(context_key: str | None) -> None:
    """Expose ATW session identity to later Claude Code Bash commands.
    ...
    *last* existing ATW_CONTEXT_ID export already assigns this value. Last
    ...
    if not env_file:
        return
    export_line = f"export ATW_CONTEXT_ID={shlex.quote(context_key)}"
    try:
...
def _last_context_key_export(env_file: str) -> str | None:
    """Return the last `export ATW_CONTEXT_ID=` line in env_file, if any.
    ...
    try:
        with open(env_file, "r", encoding="utf-8", errors="replace") as handle:
            for raw_line in handle:
                ...
                if stripped.startswith("export ATW_CONTEXT_ID="):
...
def _resolve_update_hint(atw_dir: Path, context_key: str | None) -> str | None:
    """Ask common.session_context whether an ATW update is available.

    Throttling lives there: the first SessionStart of a session writes a marker
    under `.atw/.runtime/`, and later ones (clear, compact) return without
    spawning `atw --version`. The resolved `context_key` is passed through so
    ...
    try:
        ...
        return get_update_hint(atw_dir.parent, context_key)
    except Exception:
        return None  # Optional reminder; keep session-start non-fatal.


def _resolve_active_task(atw_dir: Path, input_data: dict):
    scripts_dir = atw_dir / "scripts"
    if str(scripts_dir) not in sys.path:
        sys.path.insert(0, str(scripts_dir))
...
def run_script(script_path: Path, context_key: str | None = None) -> str:
    try:
        if script_path.suffix == ".py":
            ...
            if context_key:
                env["ATW_CONTEXT_ID"] = context_key
...
def _normalize_task_ref(task_ref: str) -> str:
    ...
    if normalized.startswith("tasks/"):
        return f".atw/{normalized}"

    return normalized


def _resolve_task_dir(atw_dir: Path, task_ref: str) -> Path:
    normalized = _normalize_task_ref(task_ref)
    path_obj = Path(normalized)
    if path_obj.is_absolute():
        return path_obj
    if normalized.startswith(".atw/"):
        return atw_dir.parent / path_obj
    return atw_dir / "tasks" / path_obj


def _get_task_status(atw_dir: Path, input_data: dict) -> str:
    """Return compact active-task status, artifact presence, and next action."""
    active = _resolve_active_task(atw_dir, input_data)

    if not active.task_path:
        return (
            "Status: NO ACTIVE TASK\n"
            "Next-Action: Classify the current turn before creating any ATW task. "
            "Simple conversation / small task asks only whether this turn should create an ATW task. "
            "Complex task asks whether task creation and planning are allowed."
        )

    task_ref = active.task_path
    task_dir = _resolve_task_dir(atw_dir, task_ref)
    if active.stale or not task_dir.is_dir():
        return (
            f"Status: STALE POINTER\nTask: {task_ref}\n"
            f"Next-Action: Run `python3 ./.atw/scripts/task.py finish` to clear the stale pointer, "
    ...
    artifact_names = ("prd.md", "implement.jsonl", "check.jsonl")
    ...
    if task_status == "completed":
        return (
            ...
            "Next-Action: The archive was interrupted — re-run `task.py archive <task>` to finish it. "
            "If the working tree is dirty, commit it or set the task back to `implement` first; "
            "there is no wrap-up commit step."
        )
    ...
    return (
        ...
        # D32/C23: the injection hook stopped materializing `design.md` /
        # `implement.md`, so naming them here would tell the agent its context
        # holds two files that are never in it.
        "Implementation/check context order is jsonl entries -> `prd.md`."
    )


def _load_atw_config(atw_dir: Path, input_data: dict) -> tuple:
    """Load ATW config for session-start decisions.
...
def _check_legacy_spec(atw_dir: Path, is_mono: bool, packages: dict) -> str | None:
...
def _collect_spec_index_paths(atw_dir: Path, allowed_pkgs: set | None) -> list[str]:
    paths: list[str] = []
    guides_index = atw_dir / "spec" / "guides" / "index.md"
    if guides_index.is_file():
        paths.append(".atw/spec/guides/index.md")
    ...
    for sub in sorted(spec_dir.iterdir()):
        ...
        if index_file.is_file():
            paths.append(f".atw/spec/{sub.name}/index.md")
            continue
        ...
        for nested in sorted(sub.iterdir()):
            ...
            if nested_index.is_file():
                paths.append(f".atw/spec/{sub.name}/{nested.name}/index.md")
...
def _build_compact_current_state(
    ...
    active = _resolve_active_task(atw_dir, input_data)
    if active.task_path:
        task_dir = _resolve_task_dir(atw_dir, active.task_path)
        status = "unknown"
    ...
    if get_tasks_dir and iter_active_tasks:
        try:
            ...
            lines.append(
                f"Active tasks: {task_count} total. Use `python3 ./.atw/scripts/task.py list --mine` only if needed."
            )
        except Exception:
...
def _strip_breadcrumb_tag_blocks(content: str) -> str:
    ...
    The tag blocks live inside `## Phase Index`, colocated with their phase
    summaries, and are consumed by the UserPromptSubmit hook
    (`inject-workflow-state.py`). The session-start payload already covers
    the full step bodies, so re-inlining the breadcrumbs here would just
    duplicate context.
    """
...
def _build_workflow_overview(workflow_path: Path) -> str:
    ...
    out_lines = [
        ...
        "Full guide: .atw/workflow.md. Step detail: `python3 ./.atw/scripts/get_context.py --mode phase --step <X.Y>`.",
        "",
    ]
...
def main():
    ...
    atw_dir = project_dir / ".atw"
    context_key = _resolve_context_key(atw_dir, hook_input)
    _persist_context_key_for_bash(context_key)

    # Load config for scope filtering and legacy detection
    is_mono, packages, scope_config, task_pkg, default_pkg = _load_atw_config(
        atw_dir,
        hook_input,
    )
    ...
    spec_index_paths = _collect_spec_index_paths(atw_dir, allowed_pkgs)

    output.write("""<session-context>
ATW compact SessionStart context. Use it to orient the session; load details on demand.
</session-context>
...
""")
    ...
    output.write(_build_compact_current_state(atw_dir, hook_input, spec_index_paths))
    output.write("\n</current-state>\n\n")

    output.write("<atw-workflow>\n")
    output.write(_build_workflow_overview(atw_dir / "workflow.md"))
    output.write("\n</atw-workflow>\n\n")

    output.write("<guidelines>\n")
    # D32/C23: same as `_get_task_status` — the injected order is jsonl then
    # `prd.md`; `design.md` / `implement.md` are no longer materialized.
    output.write(
    ...
    task_status = _get_task_status(atw_dir, hook_input)
...
Context loaded. Follow <task-status>. Load workflow/spec/task details only when needed.
    ...
    # Kiro (CLI atw agent agentSpawn) adds a hook's stdout directly to the
